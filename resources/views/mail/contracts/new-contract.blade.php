...
<body>
    <div class="wrapper">
        <div class="email-card">
            <div class="header">
                <h1>Nuevo contrato registrado</h1>
                <p>{{ $contract->codigo }}</p>
            </div>
            <div class="content">
                <p>Hola {{ trim($user->nombre ?? $user->name ?? '') ?: 'equipo' }},</p>
                <p>Se ha creado un nuevo contrato y se necesita tu apoyo para continuar con el flujo. Estos son los datos principales:</p>
                ...
                <div class="meta">
                    ...
                    <p><span class="highlight">Responsable legal:</span> {{ $contract->lawyer->nombre ?? 'Por asignar' }}</p>
                    <p><span class="highlight">Creado:</span> {{ optional($contract->created_at)->timezone(config('app.timezone', 'UTC'))->format('d/m/Y H:i') }}</p>
                </div>

                <p style="text-align: center; margin: 32px 0;">
                    <a href="{{ $ctaUrl ?? route('dashboard') }}" class="button">Ir al panel de Lextrack</a>
                </p>

                <p>Ingresa para revisar el documento, coordinar con el solicitante y definir el flujo de aprobaciones o firma según corresponda.</p>
                <p style="margin-bottom: 0;">Gracias por mantener el flujo de contratos bajo control.<br>— Equipo Lextrack</p>
            </div>
            <div class="footer">
