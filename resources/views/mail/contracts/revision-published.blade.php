...
<body>
    <div class="wrapper">
        <div class="email-card">
            <div class="header">
                <h1>Nueva revisión publicada</h1>
                <p>{{ $contract->codigo }}</p>
            </div>
            <div class="content">
                <p>Hola {{ trim($recipient->nombre ?? $recipient->name ?? '') ?: 'equipo' }},</p>
                <p>Se publicó una nueva revisión del contrato <strong>{{ $contract->titulo }}</strong> en Lextrack. Ya puedes ingresar para revisar el documento actualizado y continuar con el flujo.</p>

                <div class="meta">
                    <p><span class="highlight">Versión:</span> {{ $version->numero_version }}</p>
                    ...
                    <p><span class="highlight">Actualizado por:</span> {{ $actor->nombre ?? $actor->email ?? 'Equipo Lextrack' }}</p>
                    <p><span class="highlight">Actualizado:</span> {{ optional($version->updated_at)->format('d/m/Y H:i') ?? now()->format('d/m/Y H:i') }}</p>
                </div>
                ...
                <p>Verifica los cambios del documento y los adjuntos antes de continuar con aprobaciones, observaciones o comentarios.</p>
