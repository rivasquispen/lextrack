...
<head>
    ...
    <style>
        ...
        .comment-box { margin-top: 18px; padding: 18px; border-radius: 12px; background: #f8fafc; border-left: 4px solid #043965; color: #334155; white-space: pre-wrap; }
    </style>
</head>
<body>
    <div class="wrapper">
        <div class="email-card">
            <div class="header">
                <h1>Nuevo comentario en contrato</h1>
                <p>{{ $contract->codigo }}</p>
            </div>
            <div class="content">
                <p>Hola {{ trim($recipient->nombre ?? $recipient->name ?? '') ?: 'equipo' }},</p>
                <p>Se registró un nuevo comentario en el contrato <strong>{{ $contract->titulo }}</strong>. Revisa el mensaje para continuar con la coordinación.</p>

                <div class="meta">
                    <p><span class="highlight">Autor:</span> {{ $comment->user->nombre ?? $comment->user->email ?? 'Usuario' }}</p>
                    <p><span class="highlight">Versión:</span> {{ $comment->version->numero_version ?? 'Sin versión asociada' }}</p>
                    <p><span class="highlight">Estado:</span> {{ $contract->status_label ?? ucfirst($contract->estado) }}</p>
                    <p><span class="highlight">Fecha:</span> {{ optional($comment->created_at)->format('d/m/Y H:i') ?? now()->format('d/m/Y H:i') }}</p>
                </div>

                <div class="comment-box">{{ $comment->mensaje }}</div>

                <p style="text-align: center; margin: 32px 0;">
                    <a href="{{ $ctaUrl ?? route('contracts.show', $contract) }}" class="button">Ver comentario</a>
                </p>
