...
<body>
    <div class="wrapper">
        <div class="email-card">
            <div class="header">
                <h1>Revisión conforme para envío al proveedor</h1>
                <p>{{ $contract->codigo }}</p>
            </div>
            <div class="content">
                <p>Hola {{ trim($recipient->nombre ?? $recipient->name ?? '') ?: 'equipo' }},</p>
                <p>Se marcó una revisión del contrato <strong>{{ $contract->titulo }}</strong> como conforme para el envío al proveedor.</p>

                <div class="meta">
                    <p><span class="highlight">Marcado por:</span> {{ $actor->nombre ?? $actor->email ?? 'Equipo legal' }}</p>
                    <p><span class="highlight">Fecha de la revisión:</span> {{ optional($history->created_at)->format('d/m/Y H:i') ?? now()->format('d/m/Y H:i') }}</p>
                    <p><span class="highlight">Subido por:</span> {{ $history->uploader->nombre ?? $history->uploader->email ?? 'Usuario' }}</p>
                    <p><span class="highlight">Estado:</span> {{ $contract->status_label ?? ucfirst($contract->estado) }}</p>
                </div>

                <p style="text-align: center; margin: 32px 0;">
                    <a href="{{ $ctaUrl ?? route('contracts.show', $contract) }}" class="button">Ver contrato</a>
                </p>
